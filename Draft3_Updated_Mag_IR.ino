// updated magnet (invert directions due to sensor placement) 
// IR emission category now uses -1 for in progress, 0 for invalid result as well as a busy guard which prevents resets mid measurement
...
void setup()
{
  ...
  server.on(F("/IR"), IR_measure);
  server.on(F("/IRresult"), return_IR_result);
